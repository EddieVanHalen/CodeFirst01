...
namespace CodeFirst01.DbContext;

public partial class LibraryContext : Microsoft.EntityFrameworkCore.DbContext
{
    ...
    {
        var config = App.Provider.GetService<ConfigurationBuilder>()!;

        var path = Directory.GetCurrentDirectory();

        var newPath = Path.GetFullPath(Path.Combine(path, @"..\..\..\Config"));

        config.SetBasePath(newPath);

        var cs = config.AddJsonFile(@"config.json").Build();
